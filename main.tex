...
% ── Float placement (avoid near-empty float pages) ────────────────────────
...
% ── Abstract ─────────────────────────────────────────────────────────────────
\begin{abstract}
	Sub-lineage \lignee{L4.11} of \mtb{} (within Coll~4.9\,/\,Stucki~4.10) remains poorly characterised despite harbouring some of the highest drug-resistance rates documented in lineage~4. We performed a comprehensive phylogenomic analysis of 825~whole-genome sequences from the TBannotator~v3.2 database, identifying eight core-exclusive SNP markers and two novel sub-lineages with contrasting epidemiological profiles.

	\lignee{L4.11.1} (91~SRAs, predominantly Bangladesh) exhibits 85.7\% multidrug resistance (MDR), driven by near-fixation of \textit{katG}~S315T and \textit{rpoB}~S450L, with compensatory \textit{rpoC} mutations confirming fitness restoration. \lignee{L4.11.2} (732~SRAs, predominantly Peru) shows 27.4\% MDR but carries the \textit{mmpR5}~Ile67fs bedaquiline/clofazimine resistance variant in 24\% of strains --- currently silenced by epistatic \textit{mmpL5}~Arg202fs but representing a tangible reactivation risk. IS6110 copy numbers (median 10 in both sub-lineages) are comparable to Beijing, and IS6110-mediated disruption of the DosR dormancy regulon and the \textit{vapC45}/\textit{vapB45} toxin--antitoxin system in \lignee{L4.11.2} parallels features of the Beijing lineage (L2).

	Molecular clock analysis suggests a recent divergence (nominal TMRCA $\sim$1965), although the weak temporal signal ($R^2 = 0.008$) precludes reliable dating.  Transmission clustering (27\% of SRAs within 12-SNP clusters) confirms active recent transmission, particularly in \lignee{L4.11.2}. These findings identify \lignee{L4.11} as a lineage of major public health concern requiring enhanced molecular surveillance in South Asia and South America.
\end{abstract}
...
Among the recognised L4 sub-lineages, \lignee{L4.11} has received limited attention despite emerging evidence of clinical significance.  This sub-lineage, first identified in our TBannotator classification system~\cite{senelle2023}, overlaps partially with designations in other nomenclature systems (Table~\ref{tab:nomenclature}): it falls within the broader Coll~4.9 and Stucki~4.10 clades but is not captured at the resolution of most current barcode schemes.  Clarifying these correspondences is essential to contextualise our findings within the existing literature and to enable cross-study comparisons.
...
\begin{table}[htbp!]
	...
	\begin{tabular}{lccl}
		...
		Freschi~et~al.~(2021)~\cite{freschi2021} & \lignee{4.11} & $\subset$ & Matches our \lignee{L4.11.2} (732/825) \\
...
investigated the role of epistasis in drug resistance among 31,000~MTBC genomes,
many belonging to Freschi sub-lineage~4.11 (i.e.\ a sub-group of our
\lignee{L4.11}), and identified a monophyletic cluster from Lima, Peru
(1997--2012) harbouring co-occurring frameshift mutations in
\textit{mmpR5} (Rv0678) and \textit{mmpL5}.  \textit{mmpR5} frameshifts
confer cross-resistance to bedaquiline and clofazimine by derepressing the
MmpSL5 efflux pump; however, concomitant loss-of-function mutations in
\textit{mmpL5} or \textit{mmpS5} can restore a hypersusceptible phenotype
in certain lineages (e.g.\ subgroups of L1.1.1.1 and
L4.6)~\cite{merker2020}.  This finding underscores the need for a
fine-grained understanding of \lignee{L4.11}'s internal structure to
contextualise such resistance events.
...
Throughout this paper, we use ``SRA'' (Sequence Read Archive accession)
to denote individual genome sequences, each corresponding to a single
clinical isolate.
...
normalises them into SPDI notation~\cite{holmes2020}.
...
Additionally, TBannotator incorporates TBProfiler predictions for RIF, INH, PZA, and EMB drug-resistance~\cite{phelan2019} and allows easy extraction in a strain report, as assessed in an independent study~\cite{auganova2025}.  In the present study, drug resistance was assessed independently using SnpEff-annotated variants cross-referenced with the WHO Catalogue~v2 (see Section~\ref{sec:dr_profiling}).
...
SRAs within 100~SPDIs (i.e.\ variant positions relative to H37Rv)
of the centroid strain in the Coll~4.9
...
excluded as falling outside the \lignee{L4.11} clade.
...
predominantly from PRJEB39569 (Bangladesh), had a lower median depth
(64.5$\times$) than the \lignee{L4.11.2} SRAs from CRyPTIC projects
PRJEB41199 (125.0$\times$) and PRJEB76547 (171.9$\times$).  SRAs
from PRJNA595834 (TANDEM, Peru, Indonesia)
exhibited elevated missing-gene counts (median 34), consistent with the
...
Variants were represented in SPDI notation~\cite{holmes2020},
enabling unambiguous identification across reference coordinates.

A binary presence/absence matrix was constructed from all polymorphic
SPDI variants across all SRAs (hereafter termed the ``pan-SPDI'' matrix)
and used as input
for maximum-likelihood inference with RAxML-NG~\cite{kozlov2019} under the
BIN+$\Gamma$ model. A similar analysis was performed on SNPs only using the GTR+$\Gamma$ (General Time Reversible) model, with very similar results (not shown).  The input alignment for the global tree reconstruction included 825~\lignee{L4.11}
...
1\,095~taxa for the BIN+$\Gamma$ analysis.
...
markers from the Coll, Freschi, Napier, and Stucki classification systems~\cite{coll2014,freschi2021,napier2020,stucki2016}.

% ─── 2.3 Functional genomic analyses ─────────────────────────────────────────
\subsection{Functional genomic analyses}

\subsubsection{Drug resistance profiling}
\label{sec:dr_profiling}
...
732~\lignee{L4.11.2}, 2~basal).

Gene deletions were inferred from the variant reports
(825~SRAs), which record ``missing genes'' (genes with $<$5\%
coverage) associated with ``missing RD'' (those already named and new custom segments).  Deletions shared by $\geq$90\% of a
...
\subsubsection{In silico spoligotyping}

Spoligotype patterns were extracted from TBannotator~v3.2 variant reports
by assessing the presence or absence of each of the 43~spacers in the
direct repeat (DR) locus.  Patterns were matched to the SITVIT2
database~\cite{couvin2019} to assign Spoligotype International Types (SIT)
and SITVIT2 clade designations.
...
measure approximates a core-genome SNP distance, although formal
calibration between the two metrics has not been performed.
...
variants that arose independently on multiple branches of the maximum-likelihood tree.
...
Geographic associations were tested using $\chi^2$ tests with post-hoc
pairwise Fisher exact tests and Bonferroni correction for multiple
comparisons.  Sub-lineage-specific resistance profiles were compared
using Fisher's exact test with odds ratios and 95\% confidence
intervals.  The ratio of unique non-synonymous to synonymous variant
positions ($p_N/p_S$) was tested per gene family against the genomic
background using $2\times2$ Fisher exact tests (Bonferroni-corrected
for five comparisons).

% ─── 2.6 Molecular dating and ancestral reconstruction ──────────────────────
...
745~SRAs (90.3\%) as a discrete trait under a GTR model.
...
\begin{figure}[htbp!]
	...
	\caption{Midpoint-rooted maximum-likelihood circular phylogeny of the
		825~\lignee{L4.11} SRAs inferred with RAxML-NG (BIN+$\Gamma$ model).
		Tips are coloured by sub-lineage assignment:
		blue, \lignee{L4.11.1} ($n = 91$);
		red, \lignee{L4.11.2} ($n = 731$).
		The node separating \lignee{L4.11.1} from \lignee{L4.11.2}
		receives 100\% bootstrap support (100~replicates).
		Note the deep split between the two sub-lineages and the long
		branch lengths within \lignee{L4.11.1}, consistent with its
		higher marker count (107~core-exclusive SPDIs vs.\ 44 near-core-exclusive).}
...
\begin{figure}[htbp!]
	...
	\caption{Contextualized Lineage~4 phylogeny (181~taxa, GTR+G model, 100~bootstrap replicates). Ten representative SRAs were sampled from each of the 20~L4 sub-lineages (L4.1--L4.18, with \lignee{L4.11.1} and \lignee{L4.11.2} treated separately; $n=5$ each). A single L4.1 strain serves as the outgroup. Bootstrap support values are shown at internal nodes.}
	\label{fig:l4_context}
\end{figure}


The maximum-likelihood phylogeny of the 825~\lignee{L4.11} SRAs reveals
a well-resolved tree with two clearly separated sub-clades (bootstrap
support 100\%): \lignee{L4.11.1}
(91~SRAs, 11.0\%) and \lignee{L4.11.2} (732~SRAs, 88.7\%), plus
two deep-branching basal SRAs that lack both
sub-lineage-defining markers.
These basal SRAs --- SRR30679919 (Australia, PRJNA1161723) and
SRR33831563 (United States, PRJNA1237251) ---
occupy a topologically ancestral position.
These two SRAs share 693~SNPs (171 absent from H37Rv) and carry
...
Strikingly, the IS6110 insertions in these two basal SRAs predominantly occur in the forward orientation, contrasting with the uniformly reverse orientation observed in the rest of \lignee{L4.11}
(Section~\ref{sec:structural}).  Their intercontinental spread
(Australia--United States) and deep-branching phylogenetic position
suggest a relict sister lineage predating the divergence of the two main \lignee{L4.11} sub-lineages.
...
This basal positioning and geographic spread, together with spoligotyping evidence, suggest that the ancestor of \lignee{L4.11.1} had a wider distribution before the clonal expansion became centred on Bangladesh.
...
732~SRAs (all \lignee{L4.11.2}) are assigned to Freschi~4.11, while
the remaining 93~SRAs (\lignee{L4.11.1} and basal) fall at the
...
H37Rv, and the designation ``4.11'' was incorrectly attributed to a Peruvian epidemic sub-clade.  None of the current classification systems resolve the full \lignee{L4.11} clade as a distinct entity; where partial overlap exists, descriptions refer to a limited epidemic clone rather than the complete lineage.  Notably, the \textit{mmpL5} frameshift variant at position 777\,875:AC:A is found exclusively in a sub-branch of \lignee{L4.11.2} representing its most recently emerged subclone, within which the maximum pairwise distance never exceeds 90~SNPs (mean: $34 \pm 12$~SNPs)~\cite{vargas2023,freschi2021}.

\paragraph{Sub-lineage \lignee{L4.11.1}.}

Within \lignee{L4.11}, a robust sub-lineage emerges from the
phylogenetic analysis: \lignee{L4.11.1}.  \lignee{L4.11.1} \textit{sensu lato}, (i.e. including the 7 proto \lignee{L4.11.1}) comprises 91~SRAs (11.0\% of
...
\begin{table}[htbp]
	...
	\begin{tabular}{lr}
		...
		\midrule
		Number of SRAs    & 91                           \\
		Proportion of \lignee{L4.11}  & 11.0\%                       \\
		Shared SNPs           & 530                          \\
...
further characterising this sub-lineage.

\paragraph{Sub-lineage \lignee{L4.11.2}.}

The complement of \lignee{L4.11.1} within \lignee{L4.11} forms a second
well-supported sub-lineage.  \lignee{L4.11.2} comprises 732~SRAs
(88.7\% of \lignee{L4.11}) and is defined by 44~near-core-exclusive markers
(Table~\ref{tab:l4112}).

\begin{table}[htbp]
	\centering
	\caption{Near-core-exclusive SPDIs defining \lignee{L4.11.2}
		(44~markers; exclusivity relaxed to $\pm$1--2~SPDIs across
		$\sim$730~SRAs to account for sequencing read errors).}
	\label{tab:l4112}
	\scriptsize
	\begin{tabular}{rllllr}
		\toprule
		\textbf{Pos.} & \textbf{SPDI}      & \textbf{Gene}        & \textbf{Effect} & \textbf{Product}                  & \textbf{$n$} \\
		\midrule
		\multicolumn{6}{l}{\textit{Original 16 strict core-exclusive markers}} \\
		\addlinespace
		130\,083      & \spdi{130082:A:C}  & \textit{ctpI}        & syn             & Cation-transporter ATPase I       & 734 \\
		613\,989      & \spdi{613988:T:C}  & \textit{gabP}        & S318P           & GABA permease                     & 734 \\
		761\,889      & \spdi{761888:G:C}  & \textit{rpoB}        & V695L           & RNA polymerase $\beta$ subunit    & 734 \\
		812\,339      & \spdi{812338:T:C}  & \textit{rplE}        & V94A            & 50S ribosomal protein L5          & 735 \\
		1\,285\,148   & \spdi{1285147:G:A} & \textit{pimE}        & G53S            & PIM mannosyltransferase           & 736 \\
		2\,223\,214   & \spdi{2223213:T:G} & ---                  & intergenic      & \textit{mpt64}/Rv1979c region     & 735 \\
		2\,333\,753   & \spdi{2333752:T:G} & Rv2077c              & Q181P           & Transmembrane protein             & 738 \\
		2\,827\,364   & \spdi{2827363:G:A} & \textit{orn}         & D70N            & Oligoribonuclease                 & 732 \\
		2\,982\,158   & \spdi{2982157:C:T} & Rv2664               & A21V            & Hypothetical protein              & 734 \\
		3\,065\,517   & \spdi{3065516:G:A} & Rv2752c              & syn             & Conserved hypothetical            & 734 \\
		3\,340\,871   & \spdi{3340870:G:T} & \textit{ppk1}        & G340C           & Polyphosphate kinase              & 741 \\
		3\,505\,708   & \spdi{3505707:A:C} & \textit{fadE24}      & I116L           & Acyl-CoA dehydrogenase            & 735 \\
		3\,630\,854   & \spdi{3630853:A:T} & \textit{alkB}        & W379R           & Alkane 1-monooxygenase            & 732 \\
		4\,216\,745   & \spdi{4216744:T:C} & ---                  & intergenic      & \textit{hisC2}/Rv3771c region     & 733 \\
		4\,286\,551   & \spdi{4286550:G:A} & Rv3821               & syn             & Integral membrane protein         & 738 \\
		4\,397\,020   & \spdi{4397019:C:T} & Rv3910               & syn             & Transmembrane protein             & 733 \\
		\addlinespace
		\multicolumn{6}{l}{\textit{Additional 28 near-core-exclusive markers ($\pm$1--2 absent)}} \\
		\addlinespace
		143\,667      & \spdi{143666:C:T}  & \textit{oxcA}        & ---             & Oxalyl-CoA decarboxylase          & 731 \\
		188\,861      & \spdi{188860:G:A}  & ---                  & intergenic      & ---                               & 734 \\
		203\,704      & \spdi{203703:T:C}  & \textit{mce1D}       & ---             & Mce1 operon, lipid import         & 732 \\
		296\,325      & \spdi{296324:C:T}  & Rv0245               & ---             & Hypothetical protein              & 734 \\
		363\,279      & \spdi{363278:G:A}  & Rv0298               & ---             & Conserved hypothetical            & 743 \\
		409\,569      & \spdi{409568:G:A}  & \textit{iniB}        & ---             & Isoniazid-inducible protein B     & 733 \\
		685\,990      & \spdi{685989:CG:C} & Rv0588               & del             & Conserved hypothetical             & 733 \\
		801\,009      & \spdi{801008:A:G}  & \textit{rplC}        & syn             & 50S ribosomal protein L3          & 731 \\
		1\,211\,542   & \spdi{1211541:A:C} & ---                  & intergenic      & ---                               & 733 \\
		1\,301\,959   & \spdi{1301958:G:A} & \textit{PE12}        & ---             & PE-family protein                 & 731 \\
		1\,749\,546   & \spdi{1749545:C:G} & \textit{dnaE1}       & ---             & DNA polymerase III $\alpha$       & 729 \\
		1\,772\,016   & \spdi{1772015:G:C} & Rv1565c              & ---             & Conserved hypothetical            & 731 \\
		1\,895\,249   & \spdi{1895248:G:T} & ---                  & intergenic      & ---                               & 737 \\
		1\,910\,947   & \spdi{1910946:G:A} & Rv1685c              & ---             & Hypothetical protein              & 734 \\
		1\,959\,824   & \spdi{1959823:C:T} & ---                  & intergenic      & ---                               & 733 \\
		1\,978\,018   & \spdi{1978017:G:A} & \textit{fadD1}       & ---             & Fatty-acid-CoA synthetase         & 733 \\
		2\,111\,114   & \spdi{2111113:C:T} & Rv1863c              & ---             & Hypothetical protein              & 734 \\
		2\,649\,346   & \spdi{2649345:G:A} & \textit{phoH1}       & ---             & Phosphate starvation protein      & 733 \\
		2\,701\,141   & \spdi{2701140:G:C} & \textit{lppR}        & ---             & Lipoprotein                       & 731 \\
		3\,244\,184   & \spdi{3244183:A:G} & \textit{fadD26}      & ---             & FAAL for PDIM biosynthesis        & 733 \\
		3\,311\,738   & \spdi{3311737:G:C} & Rv2958c              & ---             & Glycosyltransferase               & 732 \\
		3\,338\,192   & \spdi{3338191:C:T} & \textit{gpdA2}       & ---             & Glycerol-3-phosphate dehydrogenase & 733 \\
		3\,419\,274   & \spdi{3419273:C:T} & Rv3058c              & ---             & Hypothetical protein              & 732 \\
		3\,821\,190   & \spdi{3821189:G:A} & Rv3402c              & ---             & Conserved hypothetical            & 735 \\
		3\,974\,446   & \spdi{3974445:G:A} & \textit{hsaG}        & ---             & Cholesterol catabolism (Hsa path.)& 732 \\
		3\,977\,383   & \spdi{3977382:A:T} & Rv3538               & ---             & Hypothetical protein              & 734 \\
		4\,299\,695   & \spdi{4299694:G:C} & ---                  & intergenic      & ---                               & 736 \\
		4\,400\,178   & \spdi{4400177:G:C} & ---                  & intergenic      & ---                               & 732 \\
		\bottomrule
	\end{tabular}
	\medskip\\
	\parbox{0.85\textwidth}{\raggedright\scriptsize
		$n$: number of SRAs carrying the marker out of $\sim$734
		\lignee{L4.11.2} SRAs.  Markers with $n < 734$ reflect ±1--2
		sequencing read errors rather than true polymorphism.  Three
		intergenic sites without annotated gene names are listed with ``---''.}
\end{table}
...
context of epistasia of drug-resistance mutations ~\cite{vargas2023}.

The functional annotation of the 44~near-core-exclusive markers reveals a
striking profile enriched in drug-target, lipid metabolism, cell-envelope,
and stress-response genes, substantially reinforcing the portrait of
\lignee{L4.11.2} as a clade with profoundly remodelled physiology.

\textit{Lipid metabolism.}  Five markers directly target lipid pathways:
\textit{fadE24} I116L (acyl-CoA dehydrogenase, $\beta$-oxidation)\cite{ramaswamy2003},
\textit{alkB} W379R (fatty acid $\omega$-hydroxylation),
\textit{fadD1} (fatty-acid-CoA synthetase, mycolic acid activation),
\textit{fadD26} (fatty-acid-AMP ligase essential for PDIM biosynthesis),
and \textit{gpdA2} (glycerol-3-phosphate dehydrogenase).
The co-occurrence of mutations in both $\beta$-oxidation (\textit{fadE24})
and PDIM/phthiocerol biosynthesis (\textit{fadD26}) is particularly
noteworthy, as PDIMs are major virulence lipids required for phagosome
escape and macrophage survival~\cite{lerner2018,rens2021}.

\textit{Cell envelope and host--pathogen interface.}  Five additional
markers perturb cell-envelope biology:
\textit{pimE} G53S (phosphatidylinositol mannoside biosynthesis~\cite{liu2025pimE}),
\textit{mce1D} (Mce1 operon, a cholesterol/lipid import system required
for intracellular growth),
\textit{PE12} (PE-family protein involved in immune evasion~\cite{sharma2022}),
\textit{iniB} (isoniazid-inducible protein~B, a marker of
cell-envelope stress~\cite{boot2017}), and
\textit{lppR} (lipoprotein, host--pathogen interface).
The mutation in \textit{mce1D} is significant: the Mce1 complex
mediates fatty acid import during intracellular infection~\cite{forrellad2014}, and its
perturbation has been linked to altered virulence and persistence
in murine models.

\textit{Drug targets and translation.}  Three markers affect the
translational and transcriptional machinery:
\textit{rpoB} V695L (RNA polymerase $\beta$ subunit, outside
the canonical RRDR),
\textit{rplE} V94A (50S ribosomal protein~L5), and
\textit{rplC} (50S ribosomal protein~L3, a linezolid target~\cite{beckert2012}).
The synonymous \textit{rplC} variant, while not expected to alter
protein sequence, may affect mRNA structure or codon usage bias.

\textit{Persistence and stress response.}  Four markers target
persistence-related pathways:
\textit{ppk1} G340C (polyphosphate kinase, essential for biofilm
and long-term macrophage survival~\cite{tiwari2019,singh2013}),
\textit{orn} D70N (oligoribonuclease, stringent response),
\textit{phoH1} (phosphate starvation-inducible protein, Pho regulon),
and \textit{hsaG} (cholesterol catabolism via the Hsa pathway,
essential for persistence in the intracellular niche).
The \textit{hsaG} mutation adds a direct link to cholesterol-dependent
intracellular survival~\cite{miner2009}, complementing the Mce1 import perturbation.

\textit{DNA replication fidelity.}  The \textit{dnaE1} mutation
targets the $\alpha$ subunit of DNA polymerase~III, the primary
replicase~\cite{rock2015}.  Alterations in replicative fidelity could influence
the mutation rate and adaptive potential of this sub-lineage.

\textit{Histidine biosynthesis.}  Together with the \textit{hisC1}~A23V
coding mutation (which defines all \lignee{L4.11}), the intergenic
variant near \textit{hisC2} constitutes a dual perturbation of
histidine biosynthesis unique to \lignee{L4.11.2}.

Taken together, these 44 markers define a lineage with concurrent
alterations across six functional axes --- lipid metabolism, cell envelope,
drug targets, persistence, replication fidelity, and amino acid
biosynthesis --- pointing toward a comprehensively remodelled
physiological programme.

\subsection{Geographic distribution}

Geographic metadata was available for 745 of 825~SRAs (90.3\%), spanning
13~countries on 6~continents.  The two sub-lineages exhibit a striking
...
\begin{table}[htbp]
	...
	\begin{tabular}{lrrr}
		...
		Australia              & 1                & 2                & 3              \\
		United Kingdom         & 2                & ---              & 2              \\
		Other (3~countries)    & ---              & 3                & 3              \\
		\midrule
		\textbf{Total (known)} & \textbf{77}      & \textbf{668}     & \textbf{745}   \\
		Unknown                & 14               & 66               & 80             \\
		\bottomrule
	\end{tabular}
\end{table}
...
Peru (83.8\% of SRAs with metadata), consistent with the Lima-based MDR
cluster described by Vargas \textit{et al.}\ and Freschi \textit{et al.}~\cite{vargas2023,freschi2021}.  In contrast,
...
A $\chi^2$ test on the 13-country contingency table
(Table~\ref{tab:geo}) yields $\chi^2 = 707.1$, $\mathrm{df} = 12$,
$p < 10^{-143}$ (Cram\'er's~$V = 0.97$), indicating a near-perfect
...
($p_{\mathrm{Bonf}} < 10^{-75}$), Peru with \lignee{L4.11.2}
($p_{\mathrm{Bonf}} < 10^{-52}$), and both South Africa
($p_{\mathrm{Bonf}} = 1.4 \times 10^{-4}$)
...
\begin{figure}[htbp!]
	...
	\caption{Geographic distribution of \lignee{L4.11} sub-lineages.
		...
		count (745~SRAs with known country out of 825).}
...
\textit{In silico} spoligotyping of all 825~SRAs reveals CRISPR spacer
deletion patterns consistent with the SNP-based sub-lineage structure
(Figure~\ref{fig:spoligo}).  Spacers~33--36 are absent in 100\% of
\lignee{L4.11} SRAs (T1 family hallmark).  Spacer~5 is uniformly absent
in \lignee{L4.11.1} and Proto-\lignee{L4.11.1} (0\%) but present in
68.4\% of \lignee{L4.11.2}, indicating an early deletion event in the
\lignee{L4.11.1} ancestor.  Spoligotype diversity is high: 382~unique
patterns among 825~SRAs, with only 3 matching a known SIT in the
SITVIT2 database~\cite{couvin2019}.  Quantitative analysis of CRISPR
spacer coverage revealed that several DR spacers (DR2, DR4--DR7) exhibit
partial coverage (0.05--0.3$\times$ genome depth) in 40--76\% of
\lignee{L4.11.2} SRAs but are uniformly absent in \lignee{L4.11.1},
suggesting ongoing mosaic erosion of the CRISPR locus specific to the
Peruvian sub-lineage.
...
\paragraph{Principal Genetic Group assignment.}
The \textit{gyrA}--\textit{katG} allelic combination at positions 7\,585 and 2\,154\,724 assigns MTBC strains to three Principal Genetic Groups (PGG)~\cite{sreevatsan1997,filliol2006}.  Depending on the epidemic cluster, \lignee{L4.11} strains belong to either PGG2 or PGG3, whereas \lignee{L4.9} strains uniformly belong to PGG3.

\subsection{Drug resistance}

Drug resistance data reveal markedly different profiles between the two \lignee{L4.11}
...
than the estimated global average ($\sim$3.3\%, WHO 2024~\cite{who2024}),
although it must be interpreted with caution given that many contributing
BioProjects specifically targeted drug-resistant isolates.

\lignee{L4.11.1} carries an exceptionally high MDR burden: 78 of
91~SRAs (85.7\%) are MDR\@.  \lignee{L4.11.2}, though more moderate,
still shows a reported MDR rate of 27.4\% (200/731).  These figures
represent upper-bound estimates driven by ascertainment bias (see
Discussion, Section~\ref{sec:limitations}) and should not be
extrapolated to population-level prevalence without comparison to
representative surveillance cohorts.
...
caution: the EMB resistance predictions are genotypic.
Cross-referencing with phenotypic MIC data from the CRyPTIC
compendium~\cite{cryptic2022} provides a partial test: among
34~\lignee{L4.11.2} SRAs in CRyPTIC lacking any
\textit{embB}/\textit{embC}/\textit{ubiA} mutation, 33 are
phenotypically EMB-susceptible (MIC\,$\leq$\,2~$\mu$g/mL) and one
is intermediate (MIC\,=\,4~$\mu$g/mL).  This argues against
widespread intrinsic EMB resistance in \lignee{L4.11.2} and suggests
that the 18~EMB-only resistant SRAs without canonical mutations
may represent genotypic false positives or harbour unrecognised
resistance determinants.  The hypothesis that lineage-fixed
cell-envelope perturbations (\textit{pimE}~G53S, \textit{fadE24},
\textit{alkB}~W379R) may confer low-level intrinsic EMB resistance
therefore remains unconfirmed by available phenotypic data~\cite{safi2019}.

Additionally, the synonymous mutation \textit{embC}~R927R
(\spdi{4243660:G:A}) is present in $>$97\% of SRAs analysed,
effectively constituting an additional \textit{de facto} lineage marker
for \lignee{L4.11}.
...
mmpL5 frameshift.
This epistatic prediction is directly confirmed by quantitative
phenotypic data: among the 47~\lignee{L4.11.2} SRAs present in the
CRyPTIC compendium~\cite{cryptic2022} (PRJEB41199), 10~carry the
double \textit{mmpR5}~Ile67fs\,+\,\textit{mmpL5}~Arg202fs genotype,
and all 9~with available bedaquiline MICs display
MIC\,$\leq$\,0.015~$\mu$g/mL (susceptible), compared with a median
MIC of 0.03~$\mu$g/mL (range 0.008--0.12) in the 37~non-carriers.
The same pattern holds for clofazimine (all carriers:
MIC\,$\leq$\,0.06~$\mu$g/mL, susceptible).  These phenotypic data
independently validate the epistatic silencing model of
Nimmo~et~al.~\cite{nimmo2024} within our \lignee{L4.11.2} dataset
and underscore the importance of considering epistatic
...
The dominant RRDR mutation is \textit{rpoB}~S450L (61.5\% \lignee{L4.11.1},
27.2\% \lignee{L4.11.2}), followed by D435Y (6.0\% \lignee{L4.11.2}) and D435V (1.8\%).
...
10): SRAs from the Bangladeshi BioProject PRJEB39569 carry 2--4~copies,
...
exhibits $>$300~unique profiles among 732~SRAs.
...
Four additional PPE mutations are \lignee{L4.11.2}-specific ($>$96\%): \textit{PPE33}
...
frameshifts fixed in \lignee{L4.11}.
...
missense variants (e.g.\ Thr445Ala, 79\%/82\% in \lignee{L4.11.1}/\lignee{L4.11.2}),
...
underscores the convergent evolution of multidrug resistance in \lignee{L4.11}.
...
Pairwise genetic distances were computed across all 825~SRAs as the
symmetric difference of their SPDI variant sets.  Because all SRAs were
called against the same reference (\spdi{NC\_000962.3}) with an identical
pipeline, each SPDI corresponds to a single variant at a defined genomic
position, and the symmetric difference counts the number of sites at
which two SRAs differ.  This metric approximates a core-genome SNP
distance, although formal calibration between the two measures has not
been performed (see Limitations).  At a threshold of
...
\lignee{L4.11.2} showed 29.7\% (217/732$+$2 basal) of SRAs in clusters,
...
the two geographic foci of \lignee{L4.11}.
...
\begin{table}[htbp!]
	...
	\caption{Sensitivity analysis of transmission clustering at different
		...
		Percentages computed over $n = 825$ (All), $n = 91$ (\lignee{L4.11.1}),
		and $n = 734$ (\lignee{L4.11.2} including 2~basal SRAs).}
	\label{tab:clustering_sensitivity}
	\begin{tabular}{rrrrrr}
		\toprule
		\textbf{Threshold} & \textbf{Pairs} & \textbf{Clustered} & \textbf{\% All} & \textbf{\% \lignee{L4.11.1}} & \textbf{\% \lignee{L4.11.2}} \\
		\midrule
...
Molecular clock analysis with TreeTime yielded a nominal TMRCA of
$\sim$\textbf{1965} for \lignee{L4.11} (bootstrap 95\%~CI: 1560--1996;
Figure~\ref{fig:clock}).
However, the root-to-tip regression on the 143~SRAs with known
collection dates yields $R^2 = 0.008$, indicating a near-absent temporal
signal.  This point estimate should be interpreted as indicating
recent divergence (likely within the last century) rather than as a
reliable date.
...
\begin{figure}[htbp!]
	...
	\caption{Root-to-tip regression for \lignee{L4.11} SRAs with known collection
		...
		regression, with a nominal TMRCA of $\sim$1965
		(bootstrap 95\%~CI: 1560--1996; $R^2 = 0.008$, near-absent
		temporal signal).}
	\label{fig:clock}
\end{figure}
...
\subsection{Phylogenetic characterisation and lineage definition}
...
\lignee{L4.11.2} (732~SRAs, 44~near-core-exclusive markers) --- substantially
...
Among the 44~near-core-exclusive markers of \lignee{L4.11.2}, the \textit{rpoB}
...
\subsection{Drug resistance: contrasting trajectories and clinical implications}
...
21 of the 57~BioProjects (contributing 41\% of the SRAs) are flagged as
MDR-enriched ($>$50\% MDR) in Table~\ref{tab:bioprojects}.
Among the most prominent, PRJEB39569 (Bangladesh, 34~SRAs, 97\% MDR)
is an MDR surveillance study,
PRJEB32234 (41~SRAs, 61\% MDR) focuses on drug-resistant lineages,
and PRJNA343736 (Peru, 86~SRAs, 67\% MDR) targets MDR isolates
from the Lima metropolitan area.
The observed MDR rates therefore represent upper-bound estimates
rather than population-level prevalence.  For comparison, the WHO
estimates national MDR/RR-TB rates at 1.9\% (new cases) and 15\%
(retreatment) in Bangladesh, and 3.0\%/15\% in
Peru~\cite{who2024} --- figures that are one to two orders of magnitude
lower than the 85.7\% and 27.4\% observed here.
...
Our cross-referencing with quantitative CRyPTIC MIC
data~\cite{cryptic2022} provides direct phenotypic confirmation:
all 9~double-mutant carriers with available bedaquiline MICs are
susceptible (MIC\,$\leq$\,0.015~$\mu$g/mL), compared with a median
of 0.03~$\mu$g/mL in non-carriers --- consistent with the
hypersusceptibility predicted by the epistatic model.
...
\textit{embB}/\textit{embC}/\textit{ubiA} mutation raised the hypothesis
...
(mycolic acid recycling) --- might collectively reduce ethambutol
permeability or target accessibility.  However, CRyPTIC phenotypic
data~\cite{cryptic2022} do not support this hypothesis: among
34~\lignee{L4.11.2} SRAs in the compendium lacking \textit{embB}
mutations, 33 are phenotypically susceptible
(MIC\,$\leq$\,2~$\mu$g/mL) and only one reaches the intermediate
threshold (MIC\,=\,4~$\mu$g/mL).  The 18~genotypically EMB-resistant
SRAs without canonical mutations therefore likely represent false-positive
genotypic predictions rather than intrinsic resistance.

\subsection{Molecular evolution: IS6110 expansion, dormancy remodelling, and convergence with Beijing}
...
of \lignee{L4.11} recapitulates the canonical MDR trajectory observed in
...
\subsection{Phylogeographic structure}

The near-complete phylogeographic partition --- \lignee{L4.11.2}
overwhelmingly South American (Peru: 80\%) and \lignee{L4.11.1} centred
on Bangladesh (81\%) --- is consistent with the monophyletic Peruvian MDR
cluster described by Vargas~et~al.~\cite{vargas2023}.
As noted in Results, the mugration-inferred root state (Peru) is driven
by sampling density rather than true dispersal history, leaving the
geographic origin of \lignee{L4.11} an open question.
The Proto-\lignee{L4.11.1} SRAs (South Africa, United Kingdom) and the
broader IS6110 hotspot analysis (dominated by Portuguese and South African
SRAs) are consistent with an alternative European dispersal route that
warrants further investigation with geographically balanced sampling.
...
study designs (e.g.\ PRJEB39569, a Bangladeshi MDR surveillance project)
rather than true population-level prevalence.  For context, the WHO
estimates national MDR/RR-TB rates at 1.9\% (new cases) and 15\%
(retreatment) in Bangladesh, and 3.0\%/15\% in Peru~\cite{who2024}.
The observed rates in our dataset (85.7\% and 27.4\%) therefore exceed
national averages by one to two orders of magnitude, confirming
substantial ascertainment bias.  These figures should be interpreted
as upper-bound estimates reflecting the MDR-enriched study designs
from which our SRAs originate, not as population-level prevalence.
...
unvalidated.  To partially address this gap, we cross-referenced our
genotypic predictions with quantitative MIC data from the CRyPTIC
compendium~\cite{cryptic2022}: 47~of the 112~CRyPTIC SRAs in our
dataset (PRJEB41199) have published MICs across 13~antibiotics.
This cross-referencing confirmed the bedaquiline epistatic silencing
(9/9 double-mutant carriers susceptible, MIC\,$\leq$\,0.015~$\mu$g/mL)
but did not support the intrinsic ethambutol resistance hypothesis
(33/34 SRAs without \textit{embB} mutations phenotypically susceptible).
The remaining 65~CRyPTIC SRAs (including all 46 from PRJEB76547)
were not present in the 2024 release of the compendium and may become
available in future updates.
We invite laboratories with access to \lignee{L4.11} clinical isolates
--- particularly from the under-characterised Bangladeshi
\lignee{L4.11.1} clade --- to complement these findings through
expanded phenotypic drug susceptibility testing.
\emph{Third}, molecular dating was performed with TreeTime under a
strict clock model.  The root-to-tip regression on the 143~retained
dated SRAs yields $R^2 = 0.008$, indicating a near-absent temporal
signal.  The nominal TMRCA of $\sim$1965 (bootstrap 95\%~CI: 1560--1996)
should therefore be regarded as a rough indication that the divergence
of \lignee{L4.11} occurred in the recent past --- likely within the
last century --- rather than as a reliable point estimate.
A Bayesian framework (e.g.\ BEAST) with relaxed clock and tip-dating
would better account for rate heterogeneity across branches, but is
unlikely to overcome the fundamental limitation of sparse temporal
sampling (143~dated SRAs over 18~years).
...
formally calibrated.  A targeted comparison of SPDI set differences
against core-genome SNP distances on a subset of isolates (e.g.\ the
112~CRyPTIC SRAs, for which independent variant calls may be available)
would establish whether the two metrics are interchangeable or whether
...
\mtb{} sub-lineage \lignee{L4.11} based on 825~genomes.  Eight
core-exclusive SNP markers define this lineage, and two novel
sub-lineages --- \lignee{L4.11.1} (91~SRAs, predominantly Bangladesh)
and \lignee{L4.11.2} (732~SRAs, predominantly Peru) --- exhibit
contrasting drug resistance trajectories, independent IS6110 expansion
patterns, and a near-complete phylogeographic partition.

Three findings carry immediate clinical significance.
\emph{First}, the high reported MDR rates (85.7\% in \lignee{L4.11.1},
27.4\% in \lignee{L4.11.2}) --- though inflated by ascertainment bias ---
combined with convergent acquisition of canonical resistance mutations
(\textit{katG}~S315T, \textit{rpoB}~S450L) across both sub-lineages,
identify \lignee{L4.11} as a lineage requiring enhanced surveillance.
\emph{Second}, the \textit{mmpR5}~Ile67fs bedaquiline resistance variant
in 24\% of \lignee{L4.11.2} --- currently silenced by epistatic
\textit{mmpL5}~Arg202fs but susceptible to reactivation --- demands
joint genotypic assessment of both loci in diagnostic algorithms.
\emph{Third}, IS6110-mediated disruption of the DosR dormancy regulon
and the \textit{vapC45}/\textit{vapB45} toxin--antitoxin system, combined
with lineage-fixed lipid metabolism perturbations, define
\lignee{L4.11.2} as a clade with a profoundly remodelled
dormancy--persistence programme that parallels features of the Beijing
lineage (L2).

The companion characterisation of sibling lineages \lignee{L4.14}~\cite{guyeuxL414},
\lignee{L4.15}~\cite{guyeuxL415}, and \lignee{L4.16} within the
Coll~4.9\,/\,Stucki~4.10 clade will complete the picture of these
cryptic sub-lineages and their diverse evolutionary strategies within
L4's ``phylogenetic dark matter''.
...
\begin{table}[htbp]
	...
	\begin{tabular}{lll}
		...
		\midrule
		SRAs                    & 91 (11.0\%)                 & 732 (88.7\%)                               \\
		Defining SPDI              & 1700209:G:A                 & 143666:C:T                                 \\
		Core-exclusive markers     & 107                         & 44 (16 strict + 28 near)                   \\
...
belong to 57~BioProjects, including PRJEB39569 (Bangladesh MDR surveillance),
PRJEB32234 and PRJNA595834 (Peru drug-resistant isolates), and PRJEB41199
and PRJEB76547 (CRyPTIC global collection~\cite{cryptic2022}).  A complete
list of SRA accession numbers and associated
...
\begin{thebibliography}{99}
	...
	\bibitem{netikul2022}
	Netikul T, Thawornwattana Y, Mahasirimongkol S, \textit{et al.}
	Whole-genome single nucleotide variant phylogenetic analysis of
	\textit{Mycobacterium tuberculosis} Lineage~1 in endemic regions of Asia and Africa.
	\textit{Scientific Reports}, 12:1565, 2022.

	\bibitem{thawornwattana2021}
	Thawornwattana Y, Mahasirimongkol S, Yanai H, \textit{et al.}
	Revised nomenclature and SNP barcode for \textit{Mycobacterium tuberculosis} lineage~2.
	\textit{Microbial Genomics}, 7(11):000697, 2021.
	...
	Phylogenetically informative mutations in genes implicated in antibiotic
	resistance in \textit{Mycobacterium tuberculosis} complex.
	\textit{Genome Medicine}, 12:27, 2020.
	...
	A comprehensive map of \textit{Mycobacterium tuberculosis} complex regions of difference.
	\textit{mSphere}, 6(4):e0053521, 2021.

	\bibitem{cingolani2012}
	Cingolani P, Platts A, Wang LL, \textit{et al.}
	A program for annotating and predicting the effects of single nucleotide
	polymorphisms, SnpEff.
	\textit{Fly}, 6(2):80--92, 2012.
	...
	\textit{Bioinformatics}, 36(6):1902--1907, 2020.
	...
	Global phylogeny of \textit{Mycobacterium tuberculosis} based on single nucleotide
	polymorphism (SNP) analysis: insights into tuberculosis evolution, phylogenetic
	accuracy of other DNA fingerprinting systems, and recommendations for a minimal
	standard SNP set.
	\textit{Journal of Bacteriology}, 188(2):759--772, 2006.
	...
	\bibitem{lerner2018}
	Lerner TR, Queval CJ, Fearns A, et al.
	Phthiocerol dimycocerosates promote access to the cytosol and intracellular burden of \textit{Mycobacterium tuberculosis} in lymphatic endothelial cells.
	\textit{BMC Biol.} 2018;16:1.

	\bibitem{rens2021}
	Rens C, Chao JD, Sexton DL, et al.
	Roles for phthiocerol dimycocerosate lipids in \textit{Mycobacterium tuberculosis} pathogenesis.
	\textit{Microbiology (Reading).} 2021;167(3):001042.

	\bibitem{forrellad2014}
	Forrellad MA, McNeil M, Santangelo M de L, et al.
	Role of the Mce1 transporter in the lipid homeostasis of \textit{Mycobacterium tuberculosis}.
	\textit{Tuberculosis (Edinb).} 2014;94(2):170--177.

	\bibitem{liu2025pimE}
	Liu Y, Brown CM, Borges N, et al.
	Mechanistic studies of mycobacterial glycolipid biosynthesis by the mannosyltransferase PimE.
	\textit{Nat Commun.} 2025;16:3927.

	\bibitem{sharma2022}
	Sharma T, Alam A, Ehtram A, et al.
	The \textit{Mycobacterium tuberculosis} PE\_PGRS protein family acts as an immunological decoy to subvert host immune response.
	\textit{Int J Mol Sci.} 2022;23(1):525.

	\bibitem{boot2017}
	Boot M, van Winden VJC, Sparrius M, et al.
	Cell envelope stress in mycobacteria is regulated by the novel signal transduction ATPase IniR in response to trehalose.
	\textit{PLoS Genet.} 2017;13(12):e1007131.

	\bibitem{beckert2012}
	Beckert P, Hillemann D, Kohl TA, et al.
	\textit{rplC} T460C identified as a dominant mutation in linezolid-resistant \textit{Mycobacterium tuberculosis} strains.
	\textit{Antimicrob Agents Chemother.} 2012;56(5):2743--2745.

	\bibitem{tiwari2019}
	Tiwari P, Gosain TP, Singh M, et al.
	Inorganic polyphosphate accumulation suppresses the dormancy response and virulence in \textit{Mycobacterium tuberculosis}.
	\textit{J Biol Chem.} 2019;294(28):10819--10832.

	\bibitem{singh2013}
	Singh R, Singh M, Arora G, et al.
	Polyphosphate deficiency in \textit{Mycobacterium tuberculosis} is associated with enhanced drug susceptibility and impaired growth in guinea pigs.
	\textit{J Bacteriol.} 2013;195(12):2839--2851.

	\bibitem{miner2009}
	Miner MD, Chang JC, Pandey AK, et al.
	Role of cholesterol in \textit{Mycobacterium tuberculosis} infection.
	\textit{Indian J Exp Biol.} 2009;47(6):407--411.

	\bibitem{rock2015}
	Rock JM, Lang UF, Chase MR, et al.
	DNA replication fidelity in \textit{Mycobacterium tuberculosis} is mediated by an ancestral prokaryotic proofreader.
	\textit{Nat Genet.} 2015;47(6):677--681.

\end{thebibliography}

\clearpage
\input{table_bioprojects}
