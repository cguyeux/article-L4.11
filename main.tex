...
\begin{abstract}
	...
	Using 825 whole-genome sequences from the TBannotator~v2 database, we
	...
	novel sub-lineages --- \lignee{L4.11.1} (91~strains, predominantly
	Bangladesh, 86\% MDR) and \lignee{L4.11.2} (731~strains, predominantly
...
covering 90~(sub-)lineages, while Shitikov~et~al.~\cite{shitikov2017}
contributed markers for the East Asian lineage and partially overlapping L4
groups.
...
at position 801\,008) corresponds to our \lignee{L4.11.2}
sub-lineage (731/825~strains), not to the full \lignee{L4.11} clade.
...
\begin{table}[htbp!]
	...
	\begin{tabular}{lccl}
		...
		\midrule
		\textbf{This study} & \lignee{L4.11} & --- & Target clade (825~strains) \\
...
recently investigated 188~isolates belonging to Freschi sub-lineage~4.11 (i.e.\
...
\textit{mmpR5} (Rv0678) and \textit{mmpL5}, conferring cross-resistance to
bedaquiline and clofazimine --- two drugs increasingly used against
multidrug-resistant TB\@.  This finding underscores the need for a fine-grained
understanding of \lignee{L4.11}'s internal structure to contextualise such
resistance events.
...
TBannotator~v2 database.  Our objectives are:
(1)~reconstruct the maximum-likelihood phylogeny and identify core-exclusive
SNPs that unambiguously define this lineage;
(2)~assess the existence of novel sub-lineages not captured by current
classification systems;
(3)~characterise the drug resistance landscape, including genotypic MDR and
pre-XDR profiles;
(4)~analyse genomic structural variation, including IS6110 copy-number
expansion, gene deletions, and RD-typing;
(5)~evaluate convergent evolution and selection pressures through homoplasy
detection and $p_N/p_S$ analysis;
(6)~estimate transmission clustering rates within and between sub-lineages; and
(7)~reconstruct the spatio-temporal history of the lineage through molecular
clock analysis and ancestral geographic inference.
...
\subsubsection{Strain selection and TBannotator platform}

All analyses were conducted using TBannotator~v2~\cite{guyeux2024tbannotator,senelle2023},
an interactive phylogenomic analysis platform for \mtb.
...
normalises them into SPDI notation (Sequence--Position--Deletion--Insertion).
...
SPDIs), and cross-referencing with multiple existing nomenclature systems
(Coll, Freschi, Napier, Stucki, Shitikov).
Additionally, TBannotator integrates TBProfiler reports~\cite{phelan2019}
for per-strain drug resistance prediction, gene deletion detection, and
IS6110 insertion site annotation.

All publicly available whole-genome sequences assigned to \lignee{L4.11} were
retrieved from the TBannotator~v2 database.
The lineage-defining marker \spdi{NC\_000962.3:1374351:C:T} was originally
identified from TBannotator~v1 through a two-step procedure: first, all
strains within a reasonable SPDI distance (i.e.\ number of variant
...
core-exclusive marker shared by all strains of the resulting monophyletic
clade was identified.
Using this marker in the v2 database, a total of 847~candidate strains
were identified.
After phylogenetic reconstruction, 825~strains were confirmed as belonging to
...
(Table~\ref{tab:bioprojects}) and reflects the expanded catalogue of TBannotator~v2,
which approximately doubles the number of indexed genomes compared with
version~1.  Complete variant reports (annotated SNPs, gene coverage,
insertion sequences, and regions of difference) were retrieved for
822~strains via the TBannotator~v2 REST API; the remaining 3~strains
lacked processed reports at the time of analysis.
All population genomic analyses (drug resistance, structural variation,
selection pressure, \textit{in silico} RD-typing) are based on this
822-strain dataset unless otherwise stated.

For phylogenetic context, up to 10~strains per non-target lineage were included,
...
Prior to phylogenetic analysis, strains were filtered using three
quality-control criteria applied automatically by the TBannotator
pipeline: (i)~GC content between 61\% and 69\% (strains outside this
range indicate potential contamination or mixed infection);
(ii)~genome coverage $\geq$95\% of the H37Rv reference; and
(iii)~mean sequencing depth $\geq$15$\times$.
Strains failing any of these criteria were excluded from further analysis.

Mapping statistics for the 822~strains that passed QC were extracted
from TBannotator~v2 variant reports.  Mean sequencing depth
ranged from 20.8$\times$ to 462.0$\times$ (median 87.0$\times$).
Genome coverage exceeded 95\% in all strains.
The depth distribution was bimodal, with 46\% of strains in the
50--100$\times$ range and 46\% in the 100--200$\times$ range;
only 3~strains (0.7\%) had depth below 30$\times$ but all maintained
$>$95\% coverage.
...
Per-BioProject comparison revealed that the \lignee{L4.11.1} strains,
predominantly from PRJEB38938 (Bangladesh), had a lower median depth
(65.3$\times$) than the \lignee{L4.11.2} strains from CRyPTIC projects
PRJEB40777 (123.9$\times$) and PRJEB31443 (113.4$\times$).  Strains
from PRJNA595834 (TANDEM, Peru) and PRJEB26924 (Damien Foundation)
...
artefacts.  No strains were excluded on the basis of sequencing quality.
...
A binary presence/absence matrix (pan-SPDI) was constructed and used as input
for maximum-likelihood inference with RAxML-NG~\cite{kozlov2019} under the
BIN+$\Gamma$ model.  The input alignment included 825~\lignee{L4.11}
strains plus up to 10~strains per non-target lineage and one outgroup.
...
\begin{itemize}
	\item \textbf{Core SPDIs}: variants present in \emph{all} strains of the clade.
	\item \textbf{Core-exclusive SPDIs}: core variants absent from all strains
	      outside the clade.
\end{itemize}

Identified SNPs were cross-referenced against existing lineage-defining
markers from Coll, Freschi, Napier, Stucki, and Shitikov classification
systems.
...
SNPs in the TBannotator~v2 variant reports for all 822~strains.
Each SNP carries SnpEff functional annotation (gene, HGVS protein
change, impact class); resistance-associated mutations were identified
...
interim).  Resistance was assessed for first-line drugs (isoniazid,
rifampicin, ethambutol, pyrazinamide) and key second-line agents
including fluoroquinolones, streptomycin, bedaquiline, and clofazimine.
Multidrug resistance (MDR) was defined as concurrent resistance to
...
\subsubsection{IS6110 copy-number estimation and genomic structural variation}

Insertion sequence profiles were extracted from TBannotator~v2
variant reports for all 822~strains.  TBannotator detects IS insertion
...
were computed for all 822~strains (91~\lignee{L4.11.1},
731~\lignee{L4.11.2}).

Gene deletions were inferred from the variant reports
(822~strains), which record ``missing genes'' (genes with $<$5\%
coverage) and ``missing RD'' (named and custom genomic segments with
insufficient coverage).  Deletions shared by $\geq$90\% of a
sub-lineage were considered lineage-defining.

\subsubsection{In silico RD-typing}

Regions of difference (RD) were typed \textit{in silico} using the
\texttt{missing\_rd} and \texttt{large\_rd} fields of the TBannotator~v2
variant reports, which catalogue 4\,850~genomic segments screened per
strain, including both named RDs (RD715, DS5, DS13, RD3, ``149'',
\textit{etc.}) and custom segments identified by the pipeline.
...
Mycobrowser and the literature.
...
all protein-coding genes with at least one variant in the 822-strain
...
Pairwise genetic distances between all 825~strains were computed as the
symmetric difference of their SPDI variant sets.  Because all strains
...
Strains within $<$12~SNPs were defined as putative transmission clusters
using single-linkage clustering.  Clustering rates were compared between
...
variants that arose independently on multiple branches of the ML tree.
Sites present in $\geq$3 strains from both sub-lineages but absent from
...
TreeTime~v0.11.4~\cite{sagulenko2018}.  Of the 223~strains with known
...
591~strains (71.6\%) as a discrete trait under a GTR model.


% ── Results ──────────────────────────────────────────────────────────────────
\section{Results}

Unless otherwise noted, phylogenetic and distance-based analyses use all
825~strains, whereas functional analyses (drug resistance, structural
variation, selection pressure, RD-typing) use the 822-strain subset with
complete TBannotator~v2 variant reports (see Section~\ref{sec:methods}).
...
\begin{figure}[htbp!]
	...
	\caption{Midpoint-rooted maximum-likelihood circular phylogeny of the
		822~\lignee{L4.11} strains inferred with RAxML-NG (GTR+G4 model).
...
\begin{figure}[htbp!]
	...
	\caption{Contextualized Lineage~4 phylogeny (181~taxa, GTR+G model, 100~bootstrap replicates). Ten representative strains were sampled from each of the 20~L4 sub-lineages (L4.1--L4.18, with L4.11.1 and L4.11.2 treated separately; $n=5$ each). A single L4.1 strain serves as the outgroup. Bootstrap support values are shown at internal nodes.}
	\label{fig:l4_context}
\end{figure}
\FloatBarrier

The maximum-likelihood phylogeny of the 825~\lignee{L4.11} strains reveals
a well-resolved tree with two clearly separated sub-clades: \lignee{L4.11.1}
(91~strains, 11.0\%) and \lignee{L4.11.2} (731~strains, 88.6\%), plus
three deep-branching basal strains (0.4\%) that lack both
sub-lineage-defining markers.
These basal strains --- SRR30679919 (Australia, PRJNA1161723),
SRR33831563 (United States, PRJNA1237251), and SRR19276639
(Peru, PRJNA707145) ---
occupy a topologically ancestral position.
The two former strains share 693~SNPs and carry
11~IS6110 copies each, with 10~of 11 insertion positions in common.
They are separated by 219~pairwise SNPs and display three
shared small-segment deletions
{\footnotesize CUS\_GS\_\allowbreak{}1155829\_\allowbreak{}1155913,
CUS\_GS\_\allowbreak{}3741905\_\allowbreak{}3742046,
CUS\_GS\_\allowbreak{}3707579\_\allowbreak{}3707631}.
Strikingly, the IS6110 insertions in these basal strains predominantly
occur in the forward orientation, contrasting with the
uniformly reverse orientation observed in the rest of \lignee{L4.11}
(Section~\ref{sec:structural}).  Their intercontinental spread
(Australia--United States) and deep-branching phylogenetic position
suggest a relict ancestral lineage predating the divergence of
the two main sub-clades.
...
\begin{figure}[htbp!]
	...
	\caption{Pairwise phylogenetic distance heatmap among \lignee{L4.11} strains
...
and East Africa.  The five South African strains originate from a
household transmission study (BioProject PRJNA1312808, ``HoTT DCTB'').
All 7~Proto-\lignee{L4.11.1} strains carry a novel IS6110 insertion
...
IS6110 insertion hotspot: 43~\lignee{L4.11.2} strains carry a nearby
reverse-oriented copy at position 2\,626\,859, and a broader database
search identifies 27~strains with forward and 15~with reverse IS6110
...
This basal positioning and geographic spread suggest that the ancestor
of \lignee{L4.11.1} had a wider distribution before the clonal
expansion centred on Bangladesh.

\begin{figure}[htbp!]
	\centering
	\includegraphics[width=0.85\textwidth]{figures/pcoa_l411.png}
	\caption{Principal Coordinates Analysis of 825~\lignee{L4.11} strains
		...
		red diamonds, \lignee{L4.11.2} and basal strains ($n = 734$).
...
A total of 214~core SPDIs were identified (present in all 822~strains).
Among these, 8~core-exclusive SPDIs are absent from all strains outside
...
Three additional markers that were core-exclusive in the initial v1 dataset
(415~strains) lost this status upon dataset enrichment: \spdi{1483362:C:T}
...
validating marker exclusivity across large, independently assembled datasets.
...
systems.  All systems assign \lignee{L4.11} strains to broadly
concordant positions within L4: Coll~4/4.9,
Napier~4/4.9, Shitikov~4/4.9, and Stucki~4.10.  In the Freschi system,
731~strains (all \lignee{L4.11.2}) are assigned to Freschi~4.11, while
the remaining 94~strains (\lignee{L4.11.1} and basal) fall at the
...
H37Rv.  None of these systems resolves the full \lignee{L4.11} clade as
a distinct entity, confirming that it represents a previously unnamed
sub-lineage.

\paragraph{Sub-lineage L4.11.1.}

Within \lignee{L4.11}, a single robust sub-lineage emerges from the
phylogenetic analysis.  \lignee{L4.11.1} comprises 91~strains (11.0\% of
\lignee{L4.11}) and is supported by 530~shared SNPs (present in all
91~strains), of which 107~are exclusive to \lignee{L4.11.1}
within the \lignee{L4.11} dataset and absent from all
\lignee{L4.11.2} strains (Table~\ref{tab:l4111}).
...
strains, confirming the robustness of this sub-lineage definition.
...
as the defining barcode for \lignee{L4.11.1}.

The core-exclusive set includes three insertion/deletion variants:
\spdi{1713356:G:GT}, \spdi{1993200:AA:C}, and \spdi{2379599:C:CA},
further distinguishing this clade.

\paragraph{Sub-lineage L4.11.2.}

The complement of \lignee{L4.11.1} within \lignee{L4.11} forms a second
well-supported sub-lineage.  \lignee{L4.11.2} comprises 731~strains
...
\lignee{L4.11.2}.  This sub-lineage likely corresponds to the clade designated
as Freschi~4.11, which was studied by Vargas~et~al.~\cite{vargas2023} in the
context of bedaquiline/clofazimine resistance.
...
Geographic metadata was available for 591 of 825~strains (71.6\%), spanning
...
\begin{table}[ht]
	...
	\caption{Geographic distribution of \lignee{L4.11} sub-lineages
		(strains with available metadata).}
	\label{tab:geo}
	\begin{tabular}{lrrr}
		\toprule
...
Peru (80.1\% of strains with metadata), consistent with the Lima-based MDR
cluster described by Vargas~et~al.~\cite{vargas2023}.  In contrast,
...
\begin{figure}[htbp!]
	...
	\caption{Geographic distribution of \lignee{L4.11} sub-lineages.
		...
		count (591~strains with known country out of 825).}
...
\textit{In silico} spoligotyping of all 825~strains reveals distinctive
CRISPR spacer deletion patterns that mirror the SNP-based sub-lineage
structure (Figure~\ref{fig:spoligo}).
The canonical spacer~33 is absent in 100\% of \lignee{L4.11} strains,
...
\lignee{L4.11.2} strains, indicating an early deletion event in the
L4.11.1 ancestor.

The most frequent spoligotype pattern in \lignee{L4.11.2}
(121~strains, 16.5\%) retains all spacers except 33--36, corresponding
to the classical T1 family.  Spoligotype diversity is high across both
sub-lineages: 382~unique patterns were observed among 825~strains, and
only 3~strains could be assigned to a known SIT\@.
...
(0.05--0.3$\times$ genome depth) in 40--76\% of \lignee{L4.11.2} strains
...
\begin{figure}[htbp!]
	...
	\caption{\textit{In silico} spoligotype analysis of \lignee{L4.11}
		...
		Red = absent in $>$95\% of strains; green = present in $>$95\%.
...
Drug resistance data reveal markedly different profiles between the two
sub-lineages.  Across all 822~\lignee{L4.11} strains with variant
...
than the global average (\textasciitilde3.3\%, WHO 2024~\cite{who2024}).

\lignee{L4.11.1} carries an exceptionally high MDR burden: 78 of
91~strains (85.7\%) are MDR\@.  \lignee{L4.11.2}, though more moderate,
still has a MDR rate of 27.4\% (200/731), considerably above the global
average.
...
events.  In \lignee{L4.11.1}, 91.2\% (83/91) of strains are INH-resistant,
...
By cross-referencing the 822~strains against the
...
\begin{figure}[htbp!]
	...
	\caption{WHO Catalogue~v2 resistance predictions for \lignee{L4.11}
		sub-lineages (822~strains).
		Bars show the percentage of strains carrying at least one
...
\end{figure}
\FloatBarrier
Across \lignee{L4.11.2}, 281~strains (38.4\%, 281/731) are EMB-resistant --- a rate
comparable to rifampicin resistance (38.0\%).  Strikingly, 40~\lignee{L4.11.2} strains
...
822~\lignee{L4.11} strains.  Codon~406
of \textit{embB} emerged as a resistance hotspot specific to
\lignee{L4.11.2}: the G406S substitution is present in 80~strains
(all \lignee{L4.11.2}, 10.9\%), while
G406D and G406A account for 30 and 11~additional EMB-R strains,
respectively.  The canonical M306I mutation was found in 123~strains
(15.0\%).  Notably, 18 of the 40~EMB-only resistant strains
...
confirmed by MIC testing; some of these 18~strains may harbour
...
(\spdi{4243660:G:A}) is present in $>$97\% of strains analysed,
...
822~strains reveals sharply divergent resistance trajectories between the
two sub-lineages.  Isoniazid resistance in \lignee{L4.11.1} is
exclusively mediated by \textit{katG}~S315T~\cite{vilcheze2014}
(91.2\%, 83/91), whereas
\lignee{L4.11.2} relies on \textit{katG}~S315T in only 32\% of strains,
...
genotype: 24.2\% of strains (177/731) carry both the \textit{mmpR5}
...
These strains therefore carry a \textit{de facto} pre-antibiotic-era
...
\lignee{L4.11.2}, yielding 356~RRDR-positive strains overall (43.3\%).
...
Among the 356~RRDR strains, compensatory \textit{rpoC} mutations are
frequent.  The most common is V483G (31~strains, all \lignee{L4.11.2}),
followed by L527V (8), and other variants.
\lignee{L4.11.1} exhibits a distinct compensatory path centred on
\textit{rpoC}~Q435P (10/91 strains, 11.0\%), a mutation not previously
described in the literature.  Three \lignee{L4.11.2}
strains additionally carry \textit{rpoA}~V183A.
...
10): strains from the Bangladeshi BioProject PRJEB38938 carry 2--4~copies,
while strains from other BioProjects carry 9--12~copies.
...
all 822~strains reveals lineage-specific deletion
blocks.

A contiguous block of four genes (Rv1758--Rv1762c) is deleted in
virtually all \lignee{L4.11.2} strains: \textit{cut1} (cutinase, 100\%),
...
(Rv3426--Rv3428c) is deleted in 29\% of \lignee{L4.11.2} strains.
...
\lignee{L4.11.1} strains --- a second toxin--antitoxin perturbation
alongside the \textit{vapC45} IS6110 disruption observed in
\lignee{L4.11.2}.
No gene exhibited a coverage ratio $\geq$\,2$\times$ the genome-wide
median in a majority of strains of either sub-lineage, indicating
...
(100\%, 822/822) and the 7-bp deletion in \textit{pks15/1} (100\%),
...
28~unique RD profiles among 91~strains, whereas \lignee{L4.11.2}
exhibits $>$300~unique profiles among 731~strains.
...
Pairwise SNP distances were computed across all 825~strains using the
symmetric difference of SPDI variant sets.  Because all strains were
...
counts the number of sites at which two strains differ.  At a threshold of
$<$12~SNPs --- widely used for recent transmission in
\textit{M.\,tuberculosis}~\cite{walker2013} --- 138~pairs were identified, grouping
226~strains (27.4\%) into 105~clusters by single-linkage.

The clustering rate differed markedly between sub-lineages:
\lignee{L4.11.2} showed 29.7\% (217/731$+$3 basal) of strains in clusters,
...
The three largest clusters comprised 6, 5, and 5~strains respectively.
...
\begin{table}[htbp!]
	...
	\caption{Sensitivity analysis of transmission clustering at different
		...
		and $n = 734$ (L4.11.2 including 3~basal strains).}
...
across all 822~strains (2{,}966~genes;
...
However, the root-to-tip regression on the 143~strains with known
...
\lignee{L4.11.1} strains were identified as clock outliers (apparent
...
\lignee{L4.11.2} strains yielded the best available temporal signal,
...
geo-localised strains), indicating that the mugration model reflects
...
\begin{figure}[htbp!]
	...
	\caption{Root-to-tip regression for \lignee{L4.11} strains with known collection
...
The enrichment from 415 to 825~strains yielded important refinements.  Three
...
\lignee{L4.11.1} (91~strains, 107~core-exclusive markers) and
\lignee{L4.11.2} (731~strains, 16~core-exclusive markers) --- substantially
...
to sampling density (Peru represents 69\% of geo-localised strains).
...
The Proto-\lignee{L4.11.1} strains
(South Africa, United Kingdom) and the broader IS6110 hotspot analysis
(dominated by Portuguese and South African strains) are also consistent
...
strains, Bangladesh and East Africa sharing \lignee{L4.11.1} --- is
...
PRJEB38938 (Bangladesh, 37~strains) is an MDR surveillance study,
PRJEB32234 (Peru, 198~strains) focuses on drug-resistant lineages,
and PRJNA595834 (Peru, 88~strains) targets MDR/XDR isolates from
diabetes-comorbid patients.  Together, these three BioProjects --- all
designed to enrich for resistant strains --- contribute 39\% of the
...
strains is particularly noteworthy.  This codon is a recognised
...
of 40~strains with EMB resistance \textit{in the absence of} INH or RIF
...
MDR\@.  The 18~EMB-only resistant strains without any canonical
...
on a panel of \lignee{L4.11.2} strains without \textit{embB} mutations.
...
in 24\% of \lignee{L4.11.2} strains merits particular discussion.
...
strains --- triggered by the ancestral IS6110 at position 1\,987\,702 ---
...
(median 10~copies) is comparable to Beijing strains (typically
...
immune-evasion-driven diversification documented in Beijing strains.
Importantly, although both lineages share the canonical
\textit{pks15/1} 7-bp deletion common to all lineage~4 strains,
...
of \lignee{L4.11} to random samples of 200~strains from three other
lineages in the TBannotator~v2 database: \lignee{L4.1}
...
relevant for the ethambutol-resistant strains lacking canonical
...
branches.  Our root-to-tip regression on the 143~retained dated strains yields
...
TBannotator~v2 database.  Eight core-exclusive markers
unambiguously define this lineage, and two novel sub-lineages ---
\lignee{L4.11.1} (91~strains, 107~core-exclusive markers,
predominantly Bangladesh, 86\% MDR) and \lignee{L4.11.2} (731~strains,
...
Notably, seven ``Proto-\lignee{L4.11.1}'' strains from South Africa
...
confirm fitness restoration in MDR strains, while a genome-wide
...
strains within 12-SNP clusters) demonstrates ongoing recent transmission,
...
from the NCBI Sequence Read Archive (SRA).  The 825~\lignee{L4.11} strains
...
The TBannotator~v2 platform is freely accessible at
...
\begin{thebibliography}{99}
	...
	A robust SNP barcode for typing \textit{Mycobacterium tuberculosis} complex strains.
