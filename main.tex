...
Additionally, we checked that TBannotator performs as well as TBProfiler for drug-resistance prediction ~\cite{phelan2019} as assessed in an independent study ~\cite{auganova2025}.
...
822~SRAs via the TBannotator~v3.2 REST API; the remaining 3~SRAs
...
Mapping statistics for the 822~SRAs that passed QC were extracted
...
SRAs plus up to 10~SRAs per non-target lineage and one outgroup.
...
SNPs in the TBannotator~v3.2 variant reports for all 822~SRAs.
...
variant reports for all 822~SRAs.  TBannotator detects IS insertion
...
were computed for all 822~SRAs (91~\lignee{L4.11.1},
731~\lignee{L4.11.2}).

Gene deletions were inferred from the variant reports
(822~SRAs), which record ``missing genes'' (genes with $<$5\%
...
825~SRAs, whereas functional analyses (drug resistance, structural
variation, selection pressure, RD-typing) use the 822-strain subset with
complete TBannotator~v3.2 variant reports (see Section~\ref{sec:methods}).
...
\begin{figure}[htbp!]
	...
	\caption{Midpoint-rooted maximum-likelihood circular phylogeny of the
		822~\lignee{L4.11} SRAs inferred with RAxML-NG (GTR+G4 model).
...
three deep-branching basal SRAs (0.4\%) that lack both
...
The two former SRAs share 693~SNPs and carry
...
Strikingly, the IS6110 insertions in these basal SRAs predominantly
...
and East Africa.  The five South African SRAs originate from a
...
A total of 214~core SPDIs were identified (present in all 822~SRAs).
...
phylogenetic analysis.  \lignee{L4.11.1} comprises 91~SRAs (11.0\% of
...
The canonical spacer~33 is absent in 100\% of \lignee{L4.11} SRAs,
...
sub-lineages.  Across all 822~\lignee{L4.11} SRAs with variant
...
By cross-referencing the 822~SRAs against the
...
822~\lignee{L4.11} SRAs.  Codon~406
...
822~SRAs reveals sharply divergent resistance trajectories between the
...
all 822~SRAs reveals lineage-specific deletion
blocks.
...
across all 822~SRAs (2{,}966~genes;
